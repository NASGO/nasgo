...
var ip = require('ip');
var Sequence = require('./helpers/tools/sequence.js');
...
module.exports = function(options, done) {
  ...
  async.auto({
    ...
    protobuf: function (cb) {
      var protobuf = require('./helpers/tools/protobuf.js');
    ...
    connect: ['config', 'genesisblock', 'logger', 'network', function (cb, scope) {
      ...
      var requestSanitizer = require('./helpers/tools/request-sanitizer');
      var queryParser = require('./helpers/tools/express-query-int');
      ...
      scope.network.app.use(require('./helpers/tools/zscheme-express.js')(scope.scheme));
      ...
      scope.network.server.listen(scope.config.port, scope.config.address, function (err) {
        scope.logger.log("Nasgo started: " + scope.config.address + ":" + scope.config.port);

        if (!err) {
          if (scope.config.ssl.enabled) {
            scope.network.https.listen(scope.config.ssl.options.port, scope.config.ssl.options.address, function (err) {
              scope.logger.log("Nasgo https started: " + scope.config.ssl.options.address + ":" + scope.config.ssl.options.port);
    ...
    dbLite: function (cb) {
      var dbLite = require('./helpers/tools/dblite-helper.js');
    ...
    balanceCache: function (cb) {
      var BalanceManager = require('./helpers/tools/balance-manager.js')
      cb(null, new BalanceManager)
    },

    model: ['dbLite', function (cb, scope) {
      var Model = require('./helpers/tools/model.js')
    ...
    base: ['dbLite', 'bus', 'scheme', 'genesisblock', function (cb, scope) {
      ...
      var Account = require('./core/account.js');
      var Consensus = require('./core/consensus.js');
